<?php
	...
	class mainModel{
		...
		public function buscarPersonasPorDocumento($documento)
		{
			// Limpiar el dato recibido
			$documento_limpio = $this->limpiarDatos($documento);
		
			// Consultas unificadas
			$query = "
				SELECT 'Visitante' AS tipo, num_documento AS num_identificacion, tipo_documento, nombres, apellidos 
				FROM visitantes WHERE num_documento = '$documento_limpio'
				UNION
				SELECT 'Funcionario' AS tipo, num_documento AS num_identificacion, tipo_documento, nombres, apellidos 
				FROM funcionarios WHERE num_documento = '$documento_limpio'
				UNION
				SELECT 'Vigilante' AS tipo, num_documento AS num_identificacion, tipo_documento, nombres, apellidos 
				FROM vigilantes WHERE num_documento = '$documento_limpio'
				UNION
				SELECT 'Aprendiz' AS tipo, num_documento AS num_identificacion, tipo_documento, nombres, apellidos 
				FROM aprendices WHERE num_documento = '$documento_limpio'
			";
			...
			if ($resultado && $resultado->num_rows > 0) {
				...
				$mensaje = [
					...
					"mensaje" => "Se han encontrado " . count($personas) . " coincidencias para el documento proporcionado.",
					"icono" => "success",
					"tipoMensaje" => "normal",
					"datos" => $personas
				];
				...
				$mensaje = [
					...
					"mensaje" => "No se encontraron personas con el documento proporcionado.",
					"icono" => "error",
					"tipoMensaje" => "normal"
				];
			}
